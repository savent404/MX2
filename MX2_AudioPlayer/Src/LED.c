...
static LED_Trigger_Method_t LED_Trigger_Method(LED_Message_t trigger_bcd)
{
    ...
    switch (trigger_bcd)
    {
        case LED_TriggerB: trigger_mode = USR.config->TBMode; trigger_mode &= 0x07; break;
        case LED_TriggerC: trigger_mode = USR.config->TCMode; trigger_mode &= 0x07; break;
        case LED_TriggerD: trigger_mode = USR.config->TDMode; trigger_mode &= 0x07; break;
        case LED_TriggerE: trigger_mode = USR.config->TEMode; break;
        default: break;
    }
    while ((trigger_mode & (0x01 << ((using_mode = rand()%4 + 1) - 1))) == 0);
    switch (using_mode)
    {
        case LED_Trigger_Method_Static: break;
...
{
    step %= 2;
    if (!step) LED_RGB_Limited(FBankColor[0], FBankColor[1], FBankColor[2], FBankColor[3]);
    else LED_RGB_Limited(BankColor[0], BankColor[1], BankColor[2], BankColor[3]);
    osEvent evt = osMessageGet(LED_CMDHandle, step_ms);
    if (evt.status != osEventMessage) return LED_NoTrigger;
    else return evt.value.v;
}
...
static LED_Message_t LED_RGB_SoftDown(uint32_t step, uint32_t step_ms, uint32_t total_ms)
{
    ...
    if (step == 0) r = CHx_VAL(1), g = CHx_VAL(2), b = CHx_VAL(3), l = CHx_VAL(4);
